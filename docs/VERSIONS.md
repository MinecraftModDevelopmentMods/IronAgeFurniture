...
- `Major.Minor.Bug` is the functional mod version. It changes when the mod's
  own behaviour or compatibility changes.
- `Target` identifies the Minecraft version and loader without changing the
  functional version.
- The target is `MmmppL`: Minecraft major (`M`), zero-padded minor (`mm`),
...
version and loader. Release metadata, source code, `mcmod.info`, the manifest,
README, and changelog must all describe the same version.
