...
public class LegacyMigrationContractTest {
	...
	public void coremodRunsBeforeAndAfterVanillaFlattening() throws Exception {
		...
		assertTrue(transformer.contains("net.minecraft.world.level.storage.LevelStorageSource"));
		assertTrue(transformer.contains("(Ljava/io/File;Lnet/minecraft/nbt/CompoundTag;)V"));
		assertTrue(transformer.contains("net.minecraft.world.level.chunk.storage.ChunkStorage"));
		assertTrue(transformer.contains("Ljava/util/Optional;"));
		assertTrue(transformer.contains("prepareLegacyChunk"));
		assertTrue(transformer.contains("finalizeLegacyChunk"));
		assertTrue(transformer.contains("net.minecraft.util.datafix.fixes.BlockStateData"));
	...
	public void migrationCoversBlocksPlayerStorageContainersAndDroppedItems() throws Exception {
		...
		assertTrue(hook.contains("BlockStateData"));
		assertTrue(hook.contains("ensurePaddedBenchBlockEntities"));
		...
		assertTrue(events.contains("getEnderChestInventory"));
		assertTrue(events.contains("EntityJoinWorldEvent"));
		assertFalse(events.contains("ChunkEvent.Load"));
