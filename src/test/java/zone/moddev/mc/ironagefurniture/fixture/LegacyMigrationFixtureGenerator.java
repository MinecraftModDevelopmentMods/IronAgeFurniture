...
/** Writes deterministic binary NBT examples using the actual 1.12 NBT classes. */
public final class LegacyMigrationFixtureGenerator {
	...
	public static void main(String[] args) throws IOException {
		...
		write(new File(output, "iaf-1.12-red-only.nbt"), createFixture(false));
		write(new File(output, "iaf-1.12-multicolour.nbt"), createFixture(true));
	}
	...
	private static NBTTagCompound createFixture(boolean multicolour) {
		...
		root.setString("MinecraftVersion", "1.12.2");
		root.setBoolean("Multicolour", multicolour);
		root.setTag("PlayerInventory", createItems(multicolour));
	...
	private static NBTTagList createDroppedItems(boolean multicolour) {
		...
		for (String wood : BOP_WOODS) {
			...
			entity.setString("id", "minecraft:item");
	...
	private static NBTTagList createNestedContainers(boolean multicolour) {
		...
		container.setString("id", "minecraft:purple_shulker_box");
