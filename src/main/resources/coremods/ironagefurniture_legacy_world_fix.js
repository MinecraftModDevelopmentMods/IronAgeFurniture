    ...
    return {
            ...
            'transformer': function(classNode) {
                for (var methodIndex = 0; methodIndex < classNode.methods.size(); ++methodIndex) {
                    ...
                    for (var instruction = method.instructions.getFirst(); instruction !== null;
                        ...
                        var loadedFile = instruction.getPrevious();
                        if (!(storedRoot instanceof VarInsnNode) || storedRoot.getOpcode() !== Opcodes.ASTORE) {
                            continue;
                        }
                        if (!(loadedFile instanceof VarInsnNode) || loadedFile.getOpcode() !== Opcodes.ALOAD) {
                            continue;
                        }

                        var capture = new InsnList();
                        capture.add(new VarInsnNode(Opcodes.ALOAD, loadedFile.var));
                        ...
                        capture.add(new MethodInsnNode(
                                ...
                                'captureLegacyWorld',
                                '(Ljava/io/File;Lnet/minecraft/nbt/CompoundTag;)V',
                                false));
        ...
        'ironagefurniture_legacy_chunk_data': {
            ...
            'transformer': function(classNode) {
                for (var methodIndex = 0; methodIndex < classNode.methods.size(); ++methodIndex) {
                    var method = classNode.methods.get(methodIndex);
                    if (method.desc !== '(Lnet/minecraft/resources/ResourceKey;Ljava/util/function/Supplier;Lnet/minecraft/nbt/CompoundTag;Ljava/util/Optional;)Lnet/minecraft/nbt/CompoundTag;') {
                        continue;
                    }
