...
//	}
	...
	private static void registerAndAssignBlock(Block block, String fieldName) {
		RegistryObject<Block> registeredBlock = Ironagefurniture.BLOCKS.register(fieldName, () -> block);
		
	    try {
	        Field field = BlockObjectHolder.class.getDeclaredField(fieldName);
	        field.setAccessible(true);
	        field.set(null, registeredBlock);
	    } catch (NoSuchFieldException | IllegalAccessException e) {
	        throw new RuntimeException("Failed to assign block to BlockObjectHolder: " + fieldName, e);
	    }
	}
	
	
	public static void registerChairs(String[] woods, boolean shield, boolean shortStool, boolean tallStool, boolean bench, String[] netherwoods ) {
		registerChairs(woods, shield, shortStool, tallStool, bench, true, netherwoods);
	}
	
	public static void registerChairs(String[] woods, boolean shield, boolean shortStool, boolean tallStool, boolean bench, boolean log, String[] netherwoods) {

		for (String wood : woods) {
			registerAndAssignBlock(new Chair(BlockBehaviour.Properties.of(Material.WOOD)), "chair_wood_ironage_classic_" + wood);
...
//		event.getRegistry().register(new ObsideanLump(1, 10, SoundType.STONE, "obsidian_chunk"));
		...
		registerChairs(VANILLA_WOOD_TYPES,
