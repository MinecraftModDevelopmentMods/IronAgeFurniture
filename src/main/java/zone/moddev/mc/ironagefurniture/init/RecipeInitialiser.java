...
import zone.moddev.mc.ironagefurniture.api.Items.ItemDrinkware.MaterialType;
import zone.moddev.mc.ironagefurniture.api.Items.ItemDrinkware.VesselType;
...
public class RecipeInitialiser {
	...
	private static void registerCustomRecipeTypes() {
		RecipeSorter.register("ironagefurniture:side_barrel", zone.moddev.mc.ironagefurniture.api.recipes.SideBarrelRecipe.class,
			RecipeSorter.Category.SHAPELESS, "after:minecraft:shapeless");
		RecipeSorter.register("ironagefurniture:upright_barrel",
			zone.moddev.mc.ironagefurniture.api.recipes.UprightBarrelRecipe.class,
			RecipeSorter.Category.SHAPELESS, "after:minecraft:shapeless");
		RecipeSorter.register("ironagefurniture:foudre", zone.moddev.mc.ironagefurniture.api.recipes.FoudreRecipe.class,
			RecipeSorter.Category.SHAPED, "after:minecraft:shaped");
		RecipeSorter.register("ironagefurniture:pot_still",
			zone.moddev.mc.ironagefurniture.api.recipes.PotStillRecipe.class,
			RecipeSorter.Category.SHAPELESS, "after:minecraft:shapeless");
	}
	...
	private static void generateBarrelRecipes() {
		...
		WoodVariantHelper.forEachEnabledPlankVariant(new WoodVariantHelper.ItemStackVariantConsumer() {
			@Override
			public void accept(String suffix, ItemStack planks) {
				Block barrel = BlockObjectHolder.barrel_wood_ironage.get(suffix);
				...
				if (barrel != null && sideBarrel != null && !sideRecipeSuffixes.contains(suffix)) {
					...
					FurnitureFactory.AddUprightBarrelRecipe(sideBarrel, barrel);
					sideRecipeSuffixes.add(suffix);
				}

				Block foudre = BlockObjectHolder.foudre_wood_ironage.get(suffix);

				if (barrel != null && foudre != null && !foudreRecipeSuffixes.contains(suffix)) {
