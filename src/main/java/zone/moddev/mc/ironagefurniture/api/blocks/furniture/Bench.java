...
public class Bench extends BackBench {
	...
	protected void generateShapes(ImmutableList<BlockState> states) {
		...
		for (BlockState state : states) {
			...
			shapes = VoxelShapes.joinUnoptimized(shapes, getShapes(rotate(Block.box(0, 6, 1, 16, 7, 15), Direction.SOUTH))[state.getValue(DIRECTION).get2DDataValue()], IBooleanFunction.OR); // bench body

			switch (type) {
				case SINGLE:
					shapes = VoxelShapes.joinUnoptimized(shapes, getShapes(rotate(Block.box(2, 0, 4, 3, 7, 12), Direction.SOUTH))[state.getValue(DIRECTION).get2DDataValue()], IBooleanFunction.OR);//bench leg
					shapes = VoxelShapes.joinUnoptimized(shapes, getShapes(rotate(Block.box(13, 0, 4, 14, 7, 12), Direction.SOUTH))[state.getValue(DIRECTION).get2DDataValue()], IBooleanFunction.OR);//bench leg
					break;
				case LEFT:
			...
			shapes = VoxelShapes.joinUnoptimized(shapes, getShapes(rotate(Block.box(0, 2, 7, 16, 4, 9), Direction.SOUTH))[state.getValue(DIRECTION).get2DDataValue()], IBooleanFunction.OR); //bench crossbar
