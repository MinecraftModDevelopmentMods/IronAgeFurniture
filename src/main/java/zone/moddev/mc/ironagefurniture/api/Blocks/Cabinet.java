...
import zone.moddev.mc.ironagefurniture.api.SurfaceItemRules;
import zone.moddev.mc.ironagefurniture.api.VasePlantHelper;
import zone.moddev.mc.ironagefurniture.api.surface.SurfaceSettingInteraction;
import zone.moddev.mc.ironagefurniture.api.surface.SurfaceSettingInteraction.Result;
...
import zone.moddev.mc.ironagefurniture.client.particle.CandleFlameParticle;
...
public class Cabinet extends Block {
	...
	public int getLightValue(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		TileEntity tileEntity = worldIn.getTileEntity(pos);
		return tileEntity instanceof TileEntityCabinet
			&& ((TileEntityCabinet)tileEntity).getSurfaceSetting().hasCandle() ? 12 : 0;
	}
	...
	public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rand) {
		...
		zone.moddev.mc.ironagefurniture.api.surface.SurfaceSetting setting =
			((TileEntityCabinet)tileEntity).getSurfaceSetting();
		double baseX = this.getDisplayItemXOffset(state) - 0.5D;
		double baseZ = this.getDisplayItemZOffset(state) - 0.5D;

		for (zone.moddev.mc.ironagefurniture.api.surface.SurfaceSetting.Slot slot
				: zone.moddev.mc.ironagefurniture.api.surface.SurfaceSetting.Slot.values()) {
			if (SurfaceItemRules.isCandle(setting.getItem(slot))) {
				CandleFlameParticle.spawn(world, pos.getX() + baseX + setting.getX(slot),
					pos.getY() + this.getDisplayBlockSurfaceYOffset() + 0.39D,
					pos.getZ() + baseZ + setting.getZ(slot));
			}
		}
	}
