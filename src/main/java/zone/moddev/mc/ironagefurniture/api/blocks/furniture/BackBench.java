...
public class BackBench extends FurnitureBlock {
	...
	protected void generateShapes(ImmutableList<BlockState> states) {
		...
		for (BlockState state : states) {
			...
			shapes = VoxelShapes.joinUnoptimized(shapes, getShapes(rotate(Block.box(0, 6, 1, 16, 7, 15), Direction.SOUTH))[state.getValue(DIRECTION).get2DDataValue()], IBooleanFunction.OR); // base
			shapes = VoxelShapes.joinUnoptimized(shapes, getShapes(rotate(Block.box(0, 0, 0, 16, 16, 1), Direction.SOUTH))[state.getValue(DIRECTION).get2DDataValue()], IBooleanFunction.OR); // back
	...
	public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
		if (state.getBlock() != newState.getBlock() && world.getBlockEntity(pos) instanceof IInventory) {
