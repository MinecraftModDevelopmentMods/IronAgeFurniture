...
public class LightSourceSconceTorchFloor extends LightHolderSconceFloor implements ILiquidContainer {
	protected static final VoxelShape AxisAlignedBB = Block.makeCuboidShape(6.0D, 0.0D, 6.0D, 10.0D, 13.0D, 10.0D);
	...
	public void tick(BlockState state, World level, BlockPos pos, Random rnd) {
		if (CanEx() && !level.isBlockPowered(pos)) {
		...
		if (stackInHand.getItem() == LightDrop().asItem() || stackInHand.isEmpty()) {
			...
			if (!player.isCreative()) {
				if (stackInHand.getItem() == LightDrop().asItem())
					...
					player.setHeldItem(hand, stackTorch);
				}
			}
