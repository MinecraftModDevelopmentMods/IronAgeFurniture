...
public class LightSourceSconceTorchWall extends LightSourceSconceTorchFloor {
	...
	public void animateTick(BlockState state, World level, BlockPos pos, Random rand) {
		if (HasFlame()) {
				...
				Pair<Double, Double> rotated = rotate(0.6D, 0.5D, state .get(DIRECTION));
				...
				level.addParticle(ParticleTypes.SMOKE, d0 + 0.27D * (double)direction1.getXOffset(), d1 + 0.22D, d2 + 0.27D * (double)direction1.getZOffset(), 0.0D, 0.0D, 0.0D);
				level.addParticle(this.flameParticle, d0 + 0.27D * (double)direction1.getXOffset(), d1 + 0.22D, d2 + 0.27D * (double)direction1.getZOffset(), 0.0D, 0.0D, 0.0D);
				}
    }
	...
	protected void generateShapes(ImmutableList<BlockState> states) {
		 ImmutableMap.Builder<BlockState, VoxelShape> builder = new ImmutableMap.Builder<>();
	        for(BlockState state : states)
	        {
				...
				shapes = VoxelShapes.combine(shapes, getShapes(rotate(Block.makeCuboidShape(6, 3, 9, 10, 13, 13), Direction.SOUTH))[state .get(DIRECTION).getHorizontalIndex()], IBooleanFunction.OR); // torch
