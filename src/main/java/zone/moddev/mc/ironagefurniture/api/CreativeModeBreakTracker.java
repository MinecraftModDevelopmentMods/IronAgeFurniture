...
public final class CreativeModeBreakTracker {
    ...
    public void onBlockBreak(BlockEvent.BreakEvent event) {
        ...
        if (!level.isRemote && isNearLavaFallingLight(level, event.getPos()))
    ...
    private static void remember(World level, BlockPos pos) {
        BREAKS.computeIfAbsent(level, ignored -> new ArrayList<>())
                .add(new BreakRecord(pos.toImmutable(), level.getGameTime()));
    }

    private static boolean isNearLavaFallingLight(World level, BlockPos pos) {
        if (isLavaFallingLight(level.getBlockState(pos.up()).getBlock())
                || isLavaFallingLight(level.getBlockState(pos.down()).getBlock()))
            return true;

        for (Direction direction : Direction.Plane.HORIZONTAL) {
            if (isLavaFallingLight(level.getBlockState(pos .offset(direction)).getBlock()))
                return true;
        }
        return false;
    }
