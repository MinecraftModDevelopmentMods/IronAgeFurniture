...
public final class LegacyPaddedItemMigration {
	...
	public static ItemStack migrate(ItemStack original) {
		...
		CompoundNBT serialized = original.write(new CompoundNBT());
		return migrateItemCompound(serialized) ? ItemStack.read(serialized) : original;
	}
	...
	static boolean migrateItemCompound(CompoundNBT stack) {
		...
		if (stack.contains("id", 8)) {
			...
			if (Ironagefurniture.MODID.equals(id.getNamespace())) {
				...
				if (LegacyPaddedBenchIds.isLegacyPaddedPath(id.getPath())) {
					targetPath = LegacyPaddedBenchIds.modernPaddedPath(id.getPath(), colour);
		...
		for (String key : new ArrayList<>(stack.keySet())) {
